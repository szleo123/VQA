...
class ImageEncoder(nn.Module):
    ...
    def __init__(self, embed_size):
        ...
        self.fc = nn.Sequential(nn.Linear(self.vgg[-3].out_channels, embed_size),
                                nn.Tanh())

    def forward(self, image):
        with torch.no_grad():
            img_feature = self.vgg(image)
...
class Attention(nn.Module):
    def __init__(self):
        ...
        self.fc_image = nn.Linear(INPUT_SIZE, NUM_CHANNEL)
        self.fc_question = nn.Linear(INPUT_SIZE, NUM_CHANNEL)
        self.dropout = nn.Dropout(p=0.5)
        self.fc_attention = nn.Linear(NUM_CHANNEL, 1)

    def forward(self, image_fe, question_fe):
        image_hidden = self.fc_image(image_fe)
        question_hidden = self.fc_question(question_fe).unsqueeze(dim=1)
        attention = torch.tanh(image_hidden+question_hidden)
        attention = self.fc_attention(self.dropout(attention))
        attention = (torch.softmax(attention, dim=1) * image_fe).sum(dim=1) + question_fe
        return attention
...
class AttentionModel(nn.Module):
    def __init__(self,qsize,asize): 
        ...
        self.san = nn.ModuleList([Attention()]*NUM_ATTENTION)
        self.tanh = nn.Tanh()
    ...
    def forward(self, image, question):
        ...
        qst_feature = self.text_encoder(question)
        combined = qst_feature
        for attn_layer in self.san:
            combined = attn_layer(img_feature, combined)
...
if __name__ == '__main__':
    ...
    valLoader = DataLoader(
            ...
            shuffle=True,
            num_workers=NUM_WORKER)
    model = AttentionModel(trainLoader.dataset.getSize("question"), trainLoader.dataset.getSize("answer")).to(DEVICE)
